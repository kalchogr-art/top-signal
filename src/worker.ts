...
function normalizeSignal(item: Obj): Obj | null {
  ...
  // IMPORTANT V1.7.1:
  // No `signal: "HUNTER_ENTRY"` property here.
  // Matcher V7.1 interprets item.signal as a nested signal object.
  return {
    type: "HUNTER_ENTRY",
    action: "ENTRY",
...
<div class="app">
  ...
  <div class="subtitle">
    V1.7.1 · TRACKER → MATCHER → FRONTEND ODDS
  </div>
  ...
  <div class="card">
    ...
    <div class="meta">
      <div id="event" class="badge">Event —</div>
      <div id="minute" class="badge">⏱ —</div>
      <div id="hunter" class="badge">🎯 Hunter —</div>
...
async function refresh() {
  try {
    ...
    if (!target) {
      status.textContent = 'NO SECURE HUNTER TARGET';
      status.style.color = '#fbbf24';

      document.getElementById('debug').textContent =
        'Tracker: ' +
        (result?.stats?.tracker_signals ?? 0) +
        ' · Matcher Hunter: ' +
        (result?.stats?.matcher_hunter_results ?? 0);
    ...
    document.getElementById('hunter').textContent =
      target.hunterScore !== null
        ? '🎯 Hunter ' + target.hunterScore
        : '🎯 Hunter —';
    ...
    document.getElementById('odds').textContent =
      target.overOdds !== null
        ? '@ ' + Number(target.overOdds).toFixed(2)
        : '@ —';
